...
完成SPI驱动的编写。
## 2025.11.25 开始实现用户并发
完成初步用户并发读取SPI六轴传感器和IIC传感器，完成CAN总线通讯。

## 2025.12.17 开始编写虚拟ram盘块设备驱动
虚拟ram盘块设备驱动，用于模拟一个块设备，用于测试块设备驱动。
块设备驱动核心是`struct block_device_operations`结构体，定义了块设备驱动的接口，包括`open()`,`release()`,`read()`,`write()`,`ioctl()`等。

- 使用请求队列构建块设备驱动，请求队列是块设备驱动的核心，请求队列是块设备驱动与块设备设备之间的通信通道。可能会用到IO调度器，适合机械磁盘设备。
  格式化之后才会出现分区：`mkfs.vfat /dev/ramdisk` 

- 不使用请求队列IO调度算法构建块设备驱动，适用于EMMC，SD，ramdisk这样没有机械结构的块设备。可以直接访问任意一个扇区
## 2025.12.29 开始编写音频ALSA驱动
